...
###############################################################################
# User config (/etc/linav/linav.conf) – silent if absent
source /etc/linav/linav.conf 2>/dev/null || true
...
if [[ -f "$PROJECT_ROOT/src/utils/logit.sh" ]]; then
  ...
  log WARN "logit.sh not found. Using basic fallback logger."
fi
...
scan_yara() {
  ...
  local stderr_temp
  stderr_temp=$(mktemp)
  # Ensure cleanup of temp file for stderr
  ...
  if [[ $yara_exit_code -ne 0 ]]; then
    ...
    # echo "LINAV_RESULT:::YARA_ERROR:::${file_to_scan}" # Optional: report specific errors for summary
  elif [[ -n "$yara_output" ]]; then
    log INFO "YARA  : $file_to_scan → MATCH: $yara_output"
    # Report first rule name from match for summary
    echo "LINAV_RESULT:::YARA_MATCH:::${file_to_scan}:::$(echo "$yara_output" | head -n1 | awk '{print $1}')"
  else
...
yara_matches_count=$(grep -c "LINAV_RESULT:::YARA_MATCH:::" "$run_results_file" || echo 0)
clamav_infections_count=$(grep -c "LINAV_RESULT:::CLAMAV_INFECTED:::" "$run_results_file" || echo 0)
vt_malicious_reports_count=$(grep -c "LINAV_RESULT:::VT_MALICIOUS:::" "$run_results_file" || echo 0)
files_with_errors_count=$(grep -c "LINAV_RESULT:::FILE_ERROR:::" "$run_results_file" || echo 0)
...
if [[ "${yara_matches_count:-0}" -gt 0 && -f "$run_results_file" ]]; then
...
if [[ "${clamav_infections_count:-0}" -gt 0 && -f "$run_results_file" ]]; then
...
if [[ "${vt_malicious_reports_count:-0}" -gt 0 && -f "$run_results_file" ]]; then
...
if [[ "${files_with_errors_count:-0}" -gt 0 && -f "$run_results_file" ]]; then
